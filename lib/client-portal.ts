...
export type ClientPortalLoadResult =
  | { status: "linked_active"; data: ClientPortalData }
  | { status: "missing_profile" }
  | { status: "inactive_profile"; profile: ClientProfile };
...
export async function loadClientPortalData(supabase: SupabaseClient, authUserId: string): Promise<ClientPortalLoadResult> {
  ...
  if (clientProfile.status !== "active") return { status: "inactive_profile", profile: clientProfile };
