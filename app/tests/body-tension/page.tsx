...
{!done ? <section className="tension-quiz">{questions.map((q,i)=><fieldset className="tension-question" key={q.title}><legend><span>0{i+1}</span>{q.title}</legend><div>{(Object.entries(q.options) as [Kind,string][]).map(([key,text])=><label className={answers[i]===key?"selected":""} key={key}><input type="radio" name={`q${i}`} checked={answers[i]===key} onChange={()=>setAnswers({...answers,[i]:key})}/><b>{key}</b><span>{text}</span></label>)}</div></fieldset>)}<button className="tension-submit" disabled={Object.keys(answers).length!==questions.length} onClick={()=>{setDone(true);scrollTo({top:0,behavior:"smooth"})}}>看我的張力傾向 <span>→</span></button></section> : <section className="tension-result"><p className="portal-kicker">YOUR BODY TENSION TENDENCY</p><h2>{winners.length>1?"混合型｜身體正在用不只一種方式硬撐":results[winners[0]].title}</h2>{winners.map(k=><article key={k}>{winners.length>1&&<h3>{results[k].title}</h3>}<p>{results[k].copy}</p><div><strong>給你的小建議</strong><p>{results[k].tip}</p></div></article>)}<aside><p>測驗結果僅供參考，幫助你先看清楚自己目前的張力傾向。<br/>如果想針對你的類型做更完整的 4R 流程：判讀、整理、整合、接回使用，BodyFix 可以提供個人化的身體狀態整理。</p><Link className="tests-button" href="/booking?service=fascia_chain_reset_60&source=body-tension">了解 BodyFix 身體張力整理 <span>→</span></Link></aside><button className="tension-restart" onClick={()=>{setAnswers({});setDone(false)}}>重新測一次</button></section>}</div></main>}
