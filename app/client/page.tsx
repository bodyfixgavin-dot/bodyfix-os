...
function ClientShell({ children, displayName }: { children: React.ReactNode; displayName?: string }) { return <main className="portal-page client-portal-page"><div className="portal-shell client-portal-shell"><nav className="portal-nav"><Link className="portal-wordmark" href="/"><span>BF</span><strong>BodyFix OS</strong></Link><Link className="portal-admin-link client-login-link" href={LOGIN_HREF}>登入</Link></nav><section className="client-portal-hero"><p className="portal-kicker">BODYFIX CLIENT PORTAL</p><h1>BODYFIX CLIENT PORTAL</h1><p className="portal-lead">你的身體整理紀錄，從這裡接著走。</p><p className="portal-intro">查看服務摘要、居家建議與近期狀態。這些內容用來協助你理解與延續身體使用，不作為醫療建議或判定。</p>{displayName ? <p className="client-welcome">{displayName}，歡迎回來。</p> : null}</section>{children}<footer className="client-safety-note">若出現持續疼痛、明顯腫脹、麻木、無力、發燒、外傷或其他疑似醫療問題，請先尋求合格醫療專業人員評估。</footer></div></main> }
