...
export function LearnerShell({ children }: { children: ReactNode }) {
  return <main className="learner-page"><div className="learner-shell"><nav className="learner-nav"><Link className="portal-wordmark" href="/learner"><span>BF</span><strong>BodyFix 讀體場</strong></Link><div><Link href="/learner/field">讀體場</Link><Link href="/learner/atlas">方法圖譜</Link><Link href="/learner/passport">讀體星圖</Link></div></nav>{children}<footer className="learner-footer">原型階段 · 各自成樹，彼此成林；地下共生，向光循環。</footer></div></main>;
}
